<script lang="ts">
    ...
    const displayTime = $derived.by(() => {
        ...
        return {
            str: { min: minStr, sec: secStr },
            int: { min: minInt, sec: secInt }
        }
    });
...
<main class="relative w-full max-w-200 mx-auto flex flex-col justify-center items-center gap-5">
    ...
    <div class="flex flex-col justify-center items-center gap-2">
        ...
        <div class="flex justify-center items-center gap-3">
            ...
            {#if PiPManager.available}
                <button onclick={async () => { await pipManager.toggle(); pipActive = pipManager.active; }} title={pipActive ? m.pip_exit() : m.pip_enter()} class="p-1.5 button-general button-base cursor-pointer">
